...
class Path {
	...
	validateRequestParameters({
		...
		return [
			...this.validateRequestBody(headers[
	...
	validateRequestBody(contentType, requestBody) {
		...
		if (!contentType || !contentType.includes(applicationJson))
			return `Missing: ${contentType} content for request body`;
	...
	_validateParameter(parameter, requestParameters) {
		...
		if (required && !requestParameters[name])
	...
	validateParameterSchema(parameter, value) {
		...
		return this.validateParameterType(parameter, value)
			...
			|| this.validateParameterLength(parameter, value)
			|| this.validateParameterReg(parameter, value);
	}

	validateParameterReg({ in: paramIn, name, schema }, value) {
		try {
			var reg = new RegExp(schema.pattern);
			if (!reg.test(value))
				return `Invalid ${paramIn} param ${name}. Not expected value of pattten: ${schema.pattern} but received value ${value}`;
	...
	validateParameterLength({ in: paramIn, name, schema }, value) {
		try {
			if (value.length < schema.minLength
				|| value.length > schema.maxLength)
				return `Invalid ${paramIn} param ${name}. Expected value of min length: ${schema.minLength} or max length: ${schema.maxLength} but received length ${value.length}`;
